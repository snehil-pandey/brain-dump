...
def regula_fals_x(f, a, b, iteration=1):
    fa = f(a)
    fb = f(b)
    m = (a*fb - b*fa)/(fb-fa)
    r = f(m)
    ...
    if abs(r) < 1e-6:
        ...
        return regula_fals_x(f, a, m, iteration + 1)
    else:
        return regula_fals_x(f, m, b, iteration + 1)

print("=== Regula-Fals Method ===\n")
